...
float Probe::run_z_probe(const bool sanity_check/*=true*/) {
  ...
  // Double-probing does a fast probe followed by a slow probe
  ...
  #endif
    {
      ...
      //TERN0 이 부분에서 자꾸 이상한 오류 발생하면서 프린트 스탑 발생함.
...
float Probe::probe_at_point(const_float_t rx, const_float_t ry, const ProbePtRaise raise_after/*=PROBE_PT_NONE*/, const uint8_t verbose_level/*=0*/, const bool probe_relative/*=true*/, const bool sanity_check/*=true*/) {
  ...
  //xyz_pos_t npos = {rx, ry, 0}; //LINC EDIT
  /*
  ...
  else if (!position_is_reachable(npos)) {return NAN;}     // The given position is in terms of the nozzle  *///LINCSOLUTION CUSTOM CODES
  // !can_reach(npos) IS NOT WORKING WHEN THE COORDINATES ARE SETTED BY MINUS VALUES.
  // WITHIN(N,L,H): COMPARE N BY "if(N>=L && N<=H) return true; else false;"  DOESN'T WORKS. 
  // Move the probe to the starting XYZ
  do_blocking_move_to(npos, feedRate_t(XY_PROBE_FEEDRATE_MM_S)); //LINCSOLUTION MEMO

  float measured_z = NAN;
  if (!deploy()){measured_z = run_z_probe(sanity_check) + offset.z;} //여기로 접근하고 나서 에러 띄움.
  if (!isnan(measured_z)) {
    
    const bool big_raise = raise_after == PROBE_PT_NONE;
  ...
  if (isnan(measured_z)) {
    stow();
    SERIAL_ECHOLNPAIR("PROBE","FAILED");
