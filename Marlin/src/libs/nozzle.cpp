...
/**
  ...
  void Nozzle::stroke(const xyz_pos_t &start, const xyz_pos_t &end, const uint8_t &strokes) {
    ...
    #endif
    #if ENABLED(LINCSOLUTION_CUSTOM_CODES)
      //xy_pos_t goBackPos;// = current_position; //LINCSOLUTION CUSTOM CODE: Set GOBACK position to current position.
    #endif
    ...
    // Move to the starting point
      ...
      #else
        do_blocking_move_to_xy(start, 500);   //LINCSOLUTION CUSTOM CODE: Set GOBACK movement speed 500mm/s
      #endif
    #else
    ...
    LOOP_L_N(i, strokes >> 1) {
        ...
        do_blocking_move_to_x(start.x);
      #else
        do_blocking_move_to_xy(end, 300);
        do_blocking_move_to_xy(start, 300);
      #endif
    }
    TERN_(NOZZLE_CLEAN_GOBACK, do_blocking_move_to(oldpos, 500));
  }
  /**
   * @brief Stroke clean pattern
   * @details Wipes the nozzle back and forth in a linear movement
   ...
   * @param strokes number of strokes to execute
   */
  void Nozzle::stroke_with_extrusion(const xyz_pos_t &start, const xyz_pos_t &end, const uint8_t &strokes) {
    millis_t delay = 1000;
    #if ENABLED(NOZZLE_CLEAN_GOBACK)
      const xyz_pos_t oldpos = current_position;
    #endif
    ...
    LOOP_L_N(i, strokes >> 1) {
        ...
        do_blocking_move_to_x(start.x);
      #else
        do_blocking_move_to_xy(end, 400);
        do_blocking_move_to_xy(start, 400);
      #endif
    }
    ...
    if(active_extruder==0){
      if(thermalManager.degTargetHotend(0)!=0){ //If target Temperature has not setted to any temperature, no extrusion
        thermalManager.wait_for_hotend(active_extruder, false);
        unscaled_e_move(30, 20);
        current_position.e = current_position.e - 30;
